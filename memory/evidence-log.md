...
### fact — OrcaRouter の実測（2026-09-22。管理画面より）
- モデル別支出: `anthropic/claude-sonnet-5` **$0.22** / `z-ai/glm-5.3-flash` **$0.01** /
  `deepseek/deepseek-v4-flash` **$0.00**。**合計 約 $0.23**。
  プロモクレジット残 **$19.76**（3,000円分・期限 2026/9/28）→ 使用は約 $0.24。
- アプリの健全性: リクエスト **34件** / エラー率 **0.0%** /
  平均レイテンシ **40.37s** / **P99 152.60s**。
- ルーティングインサイト（`orcarouter/auto` に任せた経路）: 戦略「バランス」、
  呼び出し **16件**、使用クォータ **$0.01**、成功率 **100.0%**、推定削減額 $0.00。
  タスク種別は `code` 14件（平均難易度 **0.26**）/ `chat` 2件（**0.09**）。

### interpretation
- **「実呼び出し16回」は誤りだった。** 16 はルーター経路だけの数で、
  比較実験で明示指定した分を含めると **34件**。記事とREADMEを訂正した。
- **支出の96%（$0.22）が `claude-sonnet-5`＝難度 high と判定された呼び出し**である。
  難度は `context.py` の `difficulty = "high" if reasons else "low"` で、
  reasons は「観測できていない前提／期待と食い違った観測／裏付けの無い主張／ガードの検出」。
  つまり**Evidence の評価結果から、システムが自分で高いモデルを使う場所を絞っている**。
  設計意図（Engineering Context → モデル切り替え）が支出の内訳として観測できた。
- **自前の難度判定とルーターの難度判定が同じ方向を向いた。** ルーターは 16件を
  平均難易度 0.26（code）と判定して安いモデルを選び、成功率 100%。
- **P99 152.60s は、当初 60 秒固定にしていたタイムアウトが足りなかったことの裏付け**である。
  あわせて重要なのは、上流のエラー率が **0.0%** だったこと。
  つまり ReadTimeout は「相手が落ちた」のではなく「自分が待てなかった」。
  この2つを混同しないよう、両方の数字を並べて記録する。

### open questions
- 34件のうち、どの工程が sonnet-5 を何回呼んだかは管理画面の内訳から切り分けていない。
- Named Router への固定は未実施。上の実測が「観察」の一周目にあたる。
